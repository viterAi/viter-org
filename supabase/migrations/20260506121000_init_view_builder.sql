...
create table if not exists ar_invoices (
  ...
  status text not null check (status in ('current', 'due_1_30', 'due_31_60', 'due_61_plus')),
  follow_up_status text not null check (follow_up_status in ('todo', 'in_progress', 'followed_up')),
...
create index if not exists idx_ar_invoices_source on ar_invoices (source_id);
