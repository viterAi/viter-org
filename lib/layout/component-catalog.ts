...
export const COMPONENT_CATALOG: CatalogComponent[] = [
  ...
  {
    id: "metric_card",
    description: "Single KPI card. When you have multiple metrics, pass them as a 'metrics' array (same shape as kpi_row) — they will render as a row of cards.",
    slots: [],
    allowed_props: ["label", "value", "hint", "metrics"],
    required_data_fields: [],
    sample: { label: "Total", value: "EUR 1.4M", hint: "vs last month" },
  },
  {
    id: "attention_list",
    description: "List of items needing attention. Set label_field to a schema field name to auto-populate from row data. Optionally set detail_field for a secondary value per row.",
    slots: [],
    allowed_props: ["title", "items", "max_items", "label_field", "detail_field", "rank_by", "rank_direction", "row_key", "columns"],
    required_data_fields: [],
    sample: { title: "Needs attention", label_field: "task_name", detail_field: "status", max_items: 5 },
  },
  {
    id: "activity_feed",
    description: "Timeline-style list of activity messages.",
    slots: [],
    allowed_props: ["title", "items", "max_items", "x_field", "y_field", "group_by", "value_field"],
    required_data_fields: [],
  ...
  },
  {
    ...
    slots: [],
    allowed_props: ["title_field", "subtitle_field", "value_field", "max_items", "title", "row_key", "columns", "rank_by", "rank_direction", "actions"],
    required_data_fields: [],
