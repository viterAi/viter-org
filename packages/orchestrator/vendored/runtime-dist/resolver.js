...
export class CachedResolver {
    ...
    async resolveActor(canonicalId) {
        ...
        const { data, error } = await this.db
            ...
            .maybeSingle();
        if (error)
            throw new Error(`resolveActor(${canonicalId}): ${error.message}`);
    ...
    async resolveChannel(kind, identifier) {
        ...
        const { data, error } = await this.db
            ...
            .maybeSingle();
        if (error)
            throw new Error(`resolveChannel(${kind}:${identifier}): ${error.message}`);
        const id = data?.id ?? null;
        this.channelCache.set(k, id);
        return id;
    }
    async upsertChannel(kind, identifier, displayName) {
        ...
        if (displayName !== undefined)
            insertRow.display_name = displayName;
        const { data, error } = await this.db
            .from('channels')
            .insert(insertRow)
            .select('id')
            .single();
        if (error)
            throw new Error(`upsertChannel(${kind}:${identifier}): ${error.message}`);
        const id = data.id;
