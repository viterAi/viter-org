 ...
 */
export async function scopeByDay(input, db, opts = {}) {
...
async function scopeByDayInline(args) {
    ...
    if (error)
        throw new Error(`scopeByDay: ${error.message}`);
    if (!rows)
        return [];
    ...
    const artifactIds = Array.from(new Set(rows.map((r) => r.artifact_id)));
    ...
    const { data: activeRows, error: activeErr } = await db
        ...
        .in('facet', facets);
    if (activeErr)
        throw new Error(`scopeByDay active lookup: ${activeErr.message}`);
    const activeKey = new Set((activeRows ?? []).map((a) => `${a.artifact_id}::${a.facet}::${a.active_run_id}`));
    return rows
        .filter((r) => activeKey.has(`${r.artifact_id}::${r.facet}::${r.extraction_run_id}`))
        .map((r) => ({
        id: r.id,
...
// Time helpers
// ────────────────────────────────────────────────────────────────────
function ymdToTzMidnight(ymd, timezone) {
