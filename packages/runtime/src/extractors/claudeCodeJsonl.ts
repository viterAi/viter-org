...
export const claudeCodeJsonl: Extractor = async function* (
  artifact: L0Artifact,
  run: ExtractionRun,
  ...
  const channelIdentifier =
    (artifact.metadata?.channel_identifier as string | undefined) ??
  ...
  const userCanonical =
    (artifact.metadata?.user_canonical_id as string | undefined) ??
    'mordechai-potash';
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (lineText === undefined || !lineText.trim()) continue;

    let entry: ClaudeCodeJsonlEntry;
    try {
...
function modelToCanonical(model: string | undefined): string {
 ...
 */
function deriveChannelIdentifier(artifact: L0Artifact): string {
  const m = artifact.source_uri.match(/\.claude\/projects\/(-?[^/]+)\//);
  if (!m || m[1] === undefined) return 'unknown';
